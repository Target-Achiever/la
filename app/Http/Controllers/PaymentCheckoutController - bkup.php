...

{
...

    {
        ...
        $appData = BookAppointment::leftJoin('users','appointment.provider_id','=','users.id')
        			...
        			->select('appointment.*','users.name as provider_name','services.service','users.email','user_details.address_line_1')
...

			    {
...

					{
						...
						Mail::send('emails.user_app_confirmed', ['transaction_id' => $charge->id,'name' => $userData->name,'appInfo' =>$appData],function ($m) use ($appData,$toMail){
				            ...
				            $m->from('linkaesthtics@app.com', 'LinkAesthetics');
...

    {
        ...
        $appData = BookAppointment::leftJoin('users','appointment.provider_id','=','users.id')
        			...
        			->select('appointment.*','users.name as provider_name','services.service','users.email','user_details.address_line_1','users.user_type','provider_refund_policies.refund','provider_refund_policies.percentage_week','provider_refund_policies.percentage_days','payment_history.transaction_id')
...

        {
            ...
            try{
                    ...
                    // if($userData->user_type == 'end_user')

                    if(($user1_type == 'end_user' && $user2_type == 'prescriber') || ($user1_type == 'end_user' && $user2_type == 'non_prescriber') || ($user1_type == 'non_prescriber' && $user2_type == 'prescriber'))

                    {
                        ...
                        if($appData->payment_status == 2 && ($appData->appointment_status == 1 || $appData->appointment_status == 2))//not paid and appointment status in request(not accepted)

                            {
                                ...
                                return redirect()->back()->with('success','success|Your appointment has been cancelled.');
...

                            {
                                ...
                                $destinationAmount = $appData->service_amount - ($appData->percentage_days / 100) * $appData->service_amount;

                                if($diff > 7)//days more than a week

                                {

                                    

                                    $destinationAmount = $appData->service_amount - ($appData->percentage_week / 100) * $appData->service_amount;
                            ...
                            //---------------------------------------------------------------------
                                ...
                                return redirect()->back()->with('success','success|Your appointment has been cancelled and refund will be credited to your account within 4-5 working days');
...

                            {
                                ...
                                CommonController::insert_notification(array('action_id' => $appData->id,'type' => 1,'from' => $userData->id,'to' => $appData->provider_id,'message' => ' has canncelled your appointment.'));

                                //----------------------------------------------------------------



                                return redirect()->back()->with('success',"success|Your appointment has been cancelled. According to the consulting person's cancellation policies you will not receive any refund.");
...

                    {
                        ...
                        if($appData->payment_status == 2 && ($appData->appointment_status == 1 || $appData->appointment_status == 2))//not paid and appointment status in request(not accepted)
...

                            {
                                ...
                                return redirect()->back()->with('success',"success|Your appointment has been cancelled. As per the link aesthetics rule you will not receive any amount.");
...

                            {
                                    ...
                                    return redirect()->back()->with('success',"danger|Something went wrong please try later.");
